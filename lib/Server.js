...
var debug = require('debug')('artemis:server'),
    error = require('debug')('artemis:error'),
    warn = require('debug')('artemis:warn');
...
function recursiveRegisterPacket(packets) {
    var key, packet;
    for (key in packets) {
        if (!packets.hasOwnProperty(key) || key === 'doExpand') continue;
        packet = packets[key];
        if (packet.doExpand) recursiveRegisterPacket(packet);
        else {
            if (packet.name !== key) warn('Mismatching packet names! Key is ' + key + ', but name is ' + packet.name);
...
function Server(config) {
    ...
    server.on('listening', function() {
        ...
        // Start tick loop
        if (config.server.tickSpeed <= 0) throw new Error('Tick speed must be greater than 0');
