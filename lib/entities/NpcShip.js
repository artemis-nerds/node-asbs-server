 ...
 * @returns {Array}
 */
function pickImmunities(taunts) {
    // todo: hook
    ...
    if (immunities.length < minPick && rejected.length >= minPick) return immunities.merge(pickImmunities(rejected));
    return immunities;
}
...
function NpcShip(server, type, shipSettings) {
    ...
    this.props({
        ...
        unknown6: 3,
        eliteBits: 0x00, // todo
        eliteBitsActive: 0x00, // todo
        scanned: 0x00, // todo
        faction: type.faction, // todo maybe?
        unknown7: 4294967295,
        unknown8: 1,
        unknown9: 254,
        unknown10: 4,
    ...
    server.vesselData.then(function(data) {
        ...
        var immuneTaunts = pickImmunities(_.map(taunts, function(val, id) {
        ...
        for (var i = 0; i < immuneTaunts.length; i++) {
            ...
            immunities[taunt[0]] = immunities[taunt[1]];
        }
    });
...
NpcShip.assignHooks = function(server) {
    ...
    server.hook('npc.receiveTaunt', function(npc, player, taunt, next) {
        if (npc.isImmuneTo(taunt)) debug('NPC ' + npc.name + ' is immune to ' + taunt);
        else debug('NPC ' + npc.name + ' is not immune to ' + taunt + '!');
