...
function PlayerShip(server, type, shipId, shipSettings, stations) {
    ...
    stations.helm.on('setImpulse', function(client, packet) {
        var value = packet.velocity;
        debug('SETTING IMPULSE TO ' + value);
        self.prop('impulse', value);
    });

    this.on('tick', function(delta) {
        // Make sure we always update *something*
        this.prop('energy', this.prop('energy'));

        if (self.prop('velocity') > self.performance.topSpeed) self.prop('velocity', self.performance.topSpeed);

        // todo handle
        self.prop('velocity', self.prop('velocity') + self.prop('impulse'));
        //debug(self.prop('velocity'), self.prop('impulse'));
    });
...
PlayerShip.prototype.entityType = 0x01;
