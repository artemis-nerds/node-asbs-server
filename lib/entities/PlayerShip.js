 ...
 * @inherits Ship
 */
function PlayerShip(server, type, shipSettings, shipId, stations) {
    ...
    var allGroups = this.allGroups = ClientGroup.joinAll(true, _.values(stations));

    // Events
    // todo: move these to their stations
...
/**
 * Docks with a certain station
 * Todo
 *
 * @param {Station} station
 * @returns {Ship} this
 */
PlayerShip.prototype.dockWith = function(station) {
...
PlayerShip.assignHooks = function(server) {
    ...
    server.hook('player.enableRedAlert', function(player) {
        player.prop('redAlert', 1);
        next();
    });

    server.hook('player.disableRedAlert', function(player) {
    ...
    server.hook('player.receiveMessage', function(player, message, priority, from, next) {
        player.stations.communications.send('commsIncoming', {
            priority: priority,
            sender: from.name,
            msg: message
        });
