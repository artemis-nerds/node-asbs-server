...
function Weapons(server, ship) {
    ...
    weapons.on('loadTube', function(client, packet) {
        server.triggerHook('weapons.loadTube', packet.tube, packet.ordnance);
    });

    weapons.on('unloadTube', function(client, packet) {
        server.triggerHook('weapons.unloadTube', packet.tube);
    });

    weapons.on('convertTorpedo', function(client, packet) {
        server.triggerHook('weapons.convertTorpedo', packet.direction);
    });
...
Weapons.assignHooks = function(server) {
    ...
    server.hook('weapons.loadTube', function(tube, ordnance, next) {
        // todo
    });

    server.hook('weapons.unloadTube', function(tube, next) {
        // todo
    });

    server.hook('weapons.convertTorpedo', function(direction, next) {
        // todo
    });
};