...
function Entity(server) {
    ...
    this.on('tick', function(delta) {
        ...
        if (self._hasChanged && self.entityType) {
            server.triggerHook('entity.update', self, self._changedProps, self._getClientGroup());
        }
    });
...
Entity.prototype._getClientGroup = function() {
    if (this.clientGroup) return this.clientGroup;
    return this.parents.length ? this.parents[0]._getClientGroup() : this.server;
};

Entity.prototype.forceUpdate = function(client) {
    if (this.disabled) return;
    client = client || this.server;

    // Because force-updating is often used to create objects, we have to this object
    // before we send its children
    if (this.entityType) this.server.triggerHook('entity.update', this, this._props, client);
    ...
    for (var type in this.children) {
        ...
        for (var i = 0; i < childList.length; i++) {
            childList[i].forceUpdate(client);
        }
    }
};
...
Entity.prototype.getByType = function(type, id) {
    var children = this.children[type];
...
Entity.prototype.get = function(id) {
    for (var type in this.children) {
        if (!this.children.hasOwnProperty(type)) continue;
        var entity = this.getByType(type, id);
...
Entity.prototype['while'] = function(condition) {
    ...
    if (!whileHandlers) whileHandlers = this._whileHandlers[condition] = { start: [], end: [], running: false };
...
Entity.assignHooks = function(server) {
    ...
    server.hook('entity.update', function(entity, props, client, next) {
        if (entity.id != null) props.id = entity.id;
        client.send(entity.entityName + 'Update', props);
