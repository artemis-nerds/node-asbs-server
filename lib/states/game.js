...
module.exports = function(state, server) {
    ...
    state.hook('client.selectShip', function(client, ship, next) {
        ...
        for (var stationName in previousStations) {
            ...
            if (previousStations[stationName].clients.length) hasClients = true;
        }
        if (!gameIsRunning && !hasClients) ships[client.ship].disabled = true;
    ...
    state.hook('client.selectConsole', function(client, console, next) {
        ...
        // The ship is no longer disabled as players are on it
        ship.disabled = false;
    ...
    state.hook('client.deselectConsole', function(client, console, next) {
        ...
        // If no players are on the ship and the ship has never actually been in the game,
        // we can remove it
        if (!ship.inGame) {
            var hasClients = false;
            for (var stationName in ship.stations) {
                if (!ship.stations.hasOwnProperty(stationName)) continue;
                if (ship.stations[stationName].clients.length) {
                    hasClients = true;
                    break;
                }
            }
            if (!hasClients) ship.disabled = true;
        }
    ...
    state.hook('client.joinGame', function(client, next) {
        ...
        // Make sure the ship is never removed
        ships[client.ship].inGame = true;
